...
# --- Constantes ---
EXCEL_FILE_CLIENTES = os.path.join('Base_Clientes', 'Clientes.xlsx') 
SHEET_NAME_CLIENTES = 'Base_Clientes'

# Pasta e arquivos para RESULTADOS DAS COTAÇÕES
COTACAO_FOLDER = 'Cotações_Clientes'
EXCEL_FILE_AUTO = os.path.join(COTACAO_FOLDER, 'Cotacoes_Auto.xlsx')
EXCEL_FILE_VIDA = os.path.join(COTACAO_FOLDER, 'Cotacoes_Vida.xlsx')
EXCEL_FILE_CONSORCIO = os.path.join(COTACAO_FOLDER, 'Cotacoes_Consorcio.xlsx')
EXCEL_FILE_FROTA = os.path.join(COTACAO_FOLDER, 'Cotacoes_Frota.xlsx')
...
# --- Função para Adicionar Dados ao Excel (Base_Clientes) ---
def append_to_excel_clientes(file_path, sheet_name, data_dict):
    ...
    try:
        ...
        if not os.path.exists(file_path):
            ...
            for col_idx, header in enumerate(headers, start=2): # Coluna B
                cell = sheet.cell(row=2, column=col_idx) # Linha 2
                cell.value = header
            ...
            if sheet_name in wb.sheetnames:
                ...
                if next_row < 3: # Garante que comece abaixo do header
                    next_row = 3
            else:
        ...
        sheet.cell(row=next_row, column=2).value = data_dict['Nome']
        sheet.cell(row=next_row, column=3).value = data_dict['Documento']
        sheet.cell(row=next_row, column=4).value = data_dict['Endereço']
        sheet.cell(row=next_row, column=5).value = data_dict['E-mail']
        sheet.cell(row=next_row, column=6).value = data_dict['Data Nasc.']
        sheet.cell(row=next_row, column=7).value = data_dict['Produto Simulado']
        ...
        wb.save(file_path)
         ...
         sg.popup_error(f"Erro inesperado ao tentar salvar em Clientes.xlsx:\n{e}")

# --- NOVAS FUNÇÕES PARA SALVAR COTAÇÕES ---
def append_to_excel_cotacao(file_path, sheet_name, data_list):
    """Função genérica para adicionar uma lista de dados (linha) a uma planilha de cotação."""
    try:
        # Garante que a pasta Cotações_Clientes exista
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        
        if not os.path.exists(file_path):
            sg.popup_error(f"Erro: Arquivo {os.path.basename(file_path)} não encontrado.\nEle deve existir em '{COTACAO_FOLDER}' com os cabeçalhos na Linha 2, Coluna B.")
            return
        ...
        if sheet_name in wb.sheetnames:
            sheet = wb[sheet_name]
            next_row = sheet.max_row + 1
            if next_row < 3: # Dados sempre começam na linha 3
                next_row = 3
        else:
            sg.popup_error(f"Erro: Aba '{sheet_name}' não encontrada no arquivo {file_path}.")
            return
        
        # Adiciona os novos dados (data_list) começando da coluna B
        for col_idx, data in enumerate(data_list, start=2): # start=2 é Coluna B
            sheet.cell(row=next_row, column=col_idx).value = data
        ...
        wb.save(file_path)
         ...
         sg.popup_error(f"Erro inesperado ao tentar salvar em {file_path}:\n{e}")
...
def abrir_janela_auto(modo_frota=False, dados_basicos=None): # MODIFICADO: recebe dados_basicos
    sg.theme('LightBlue2')
        ...
        try:
            ...
            if event == '-PLACA_CHASSI-':
                ...
                if current_input_val and (not current_input_val[-1].isdigit() or len(current_input_val) > 2): window['-IDADE-'].update(current_input_val[:-1])
                 ...
                 elif text and text[-1] not in letras_acentos_espaco: window['-INPUT_SINISTRO-'].update(text[:-1])
        ...
        if event == texto_botao:
            try:
                ...
                if modo_frota: dados_cliente_veiculo = dados_cliente; break
                    ...
                    if dados_basicos:
                        data_simulacao = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
                        for cotacao in resultados:
                            linha_dado = [
                                dados_basicos['nome'], dados_basicos['documento'], data_simulacao,
                                dados_cliente['placa_chassi'], f"R$ {dados_cliente['valor_fipe']:.2f}",
                                cotacao['seguradora'], f"R$ {cotacao['preco_anual']:.2f}", f"R$ {cotacao['valor_franquia']:.2f}"
                            ]
                            append_to_excel_cotacao(EXCEL_FILE_AUTO, "Cotacoes_Auto", linha_dado)
...
def abrir_janela_vida(dados_basicos=None):
    sg.theme('LightBlue2')
    col_dados_basicos = [
            ...
            try:
                ...
                if dados_basicos:
                    ...
                    for cotacao in resultados:
                        linha_dado = [
                            dados_basicos['nome'], dados_basicos['documento'], data_simulacao,
                            f"R$ {dados_cliente['valor_segurado']:.2f}", dados_cliente['idade'],
                            cotacao['seguradora'], f"R$ {cotacao['preco_mensal']:.2f}",
                            f"R$ {cotacao['custo_base_risco']:.2f}", f"R$ {cotacao['custo_opcionais']:.2f}"
                        ]
                        append_to_excel_cotacao(EXCEL_FILE_VIDA, "Cotacoes_Vida", linha_dado)

                abrir_janela_resultados_vida(resultados)
            except Exception as e: sg.popup_error(f'Erro ao processar dados. Verifique os campos.\n{e}')
    window.close()

def abrir_janela_consorcio(dados_basicos=None): # MODIFICADO: recebe dados_basicos
    sg.theme('LightBlue2')
    layout = [
            ...
            try:
                ...
                if dados_basicos:
                    ...
                    for cotacao in resultados:
                        linha_dado = [
                            dados_basicos['nome'], dados_basicos['documento'], data_simulacao,
                            cotacao['tipo_bem'], f"R$ {cotacao['carta_credito']:.2f}", cotacao['prazo_meses'],
                            cotacao['administradora'], f"R$ {cotacao['valor_parcela']:.2f}",
                            f"R$ {cotacao['valor_lance_embutido']:.2f}", f"R$ {cotacao['valor_liquido_a_receber']:.2f}"
                        ]
                        append_to_excel_cotacao(EXCEL_FILE_CONSORCIO, "Cotacoes_Consorcio", linha_dado)

                abrir_janela_resultados_consorcio(resultados)
            except Exception as e: sg.popup_error(f'Erro ao processar dados. Verifique os campos.\n{e}')
    window.close()

def abrir_janela_frota(dados_basicos=None): # MODIFICADO: recebe dados_basicos
    sg.theme('LightBlue2')
    ...
    while True:
        ...
        if event == 'Adicionar Veículo':
            novo_veiculo = abrir_janela_auto(modo_frota=True, dados_basicos=dados_basicos) 
            if novo_veiculo:
        ...
        if event == 'Gerar Cotação da Frota':
            ...
            if dados_basicos:
                data_simulacao = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
                num_veiculos = len(lista_veiculos_frota)
                for cotacao in resultados:
                    linha_dado = [
                        dados_basicos['nome'], dados_basicos['documento'], data_simulacao,
                        num_veiculos, cotacao['seguradora'], f"R$ {cotacao['premio_total_bruto']:.2f}",
                        f"R$ {cotacao['valor_desconto']:.2f}", f"R$ {cotacao['premio_final_com_desconto']:.2f}"
                    ]
                    append_to_excel_cotacao(EXCEL_FILE_FROTA, "Cotacoes_Frota", linha_dado)
    ...
    while True:
        ...
        if event in button_events:
            ...
            if any(not values[campo] for campo in campos_basicos): sg.popup_error('Preencha os Dados Básicos...'); continue
                ...
                # CORREÇÃO: Chamando a função com o nome certo
                append_to_excel_clientes(EXCEL_FILE_CLIENTES, SHEET_NAME_CLIENTES, new_data) # Salva no Clientes.xlsx
                
                # Guarda os dados básicos para passar para as janelas de cotação
                dados_basicos_cliente = {
                    'nome': values['-NOME-'],
                    'documento': values['-CPF_CNPJ-']
                }
                ...
                # --- Abre a janela de cotação passando os dados básicos ---
                if event == '-BTN_AUTO-': abrir_janela_auto(modo_frota=False, dados_basicos=dados_basicos_cliente)
                elif event == '-BTN_FROTA-': abrir_janela_frota(dados_basicos=dados_basicos_cliente)
                elif event == '-BTN_VIDA-': abrir_janela_vida(dados_basicos=dados_basicos_cliente)
                elif event == '-BTN_CONSORCIO-': abrir_janela_consorcio(dados_basicos=dados_basicos_cliente)
    window.close()
